...
func ScanLogic(ctx context.Context, dom string) (int, ScanTask, error) {
	...
	for i := range MaxRetries {
		slog.Info("Intento numero: ", "intento", i)
		wait := 5 * time.Second
		...
		// SOLUCION DE MANEJO HTTP TEMPORAL A LA ESPERA DE SER MEJORADA
		switch statusCode {
		case 400:
		...
		case 441:
			...
			return statusCode, ScanTask{Status: "failed", Error: &errMsg}, nil
		case 429:
			slog.Warn("Demasiados intentos, disminuyendo taza de reintentos")
			// Tambien es necesario consultar cuandos clientes hay activos y permitidos para reducir el numero de trabajadores
			wait = 30 * time.Second
		case 500:
			limit500++
	...
	return 0, ScanTask{}, fmt.Errorf("Ha terminado el numero de intentos maximos, finiquitando proceso")
}
